...
const TrafficAnalysisReportInputSchema = z.object({
  trafficData: z
    .string()
    .describe('Captured network traffic data in a standard format like PCAP or JSON, converted to text. This text should include details like source/destination IPs, ports, protocols, packet summaries, and ideally TCP flags (SYN, ACK) and HTTP request details (method, URL, status) or DNS query details if available.'),
  filters: z
    .string()
    .optional()
...
const TrafficAnalysisReportOutputSchema = z.object({
  ...
  httpAndDnsAnalysis: z
    .string()
    .optional()
    .describe("A summary of observed HTTP requests (methods, URLs, hostnames, patterns) and DNS queries (queried domains, frequency, suspicious lookups). This should be based on textual data representing HTTP and DNS traffic, if present in the input. If no clear HTTP or DNS data is present or inferable, state that."),
  recommendations: z
    .string()
...
7. HTTP & DNS Analysis: If the traffic data includes details of HTTP requests (e.g., GET/POST methods, URLs, hostnames, headers, status codes from logs) and/or DNS queries (e.g., requested domain names, query types), summarize these. For HTTP, mention common destinations, types of requests, and anything suspicious. For DNS, highlight frequent lookups, queries to known malicious or suspicious domains, or unusual query types. If no clear HTTP or DNS data is present or inferable from the logs, state that this information could not be determined for the respective part.
8. Recommendations: Provide actionable recommendations to improve network security based on the analysis.
9. IP Address Frequency: Identify the top 5-10 most frequent *destination* IP addresses observed in the outbound traffic. For each, provide the IP address and its count of occurrences. Format this as an array of objects, where each object has an "ip" (string) and "count" (number) property. If the traffic data is insufficient or too sparse to determine meaningful frequency, you may return an empty array for ipFrequency or omit the field. Example: [{"ip": "8.8.8.8", "count": 42}, {"ip": "1.1.1.1", "count": 20}]
10. Summary Counts:
